...
> 리액트-리덕스는 Provider라는 컴포넌트를 제공하고 해당 컴포넌트로 App을 감싼후에
리덕스에서 생성한 store를 app으로 전달해 줄 수 있다 . 리액트-리덕스에서 제공하는 HOC 혹은 hook을 통해 리액트 컴포넌트에 리덕스를 연결시켜 줄 수 있다. 리덕스에서 관리하는 state를 컴포넌트의 props로 전달해 주기 때문에 해당역할을 해줄 컨테이너를 만들고 컨테이너에서 컴포넌트로 state를 전달해 준다 . 이떄 리덕스와의 연결을 담당할 컨테이너와 화면을 담당할 컴포넌트를 분리해서 만든다. 
